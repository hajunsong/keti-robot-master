...
class Robot_Slave(Parent_Robot):
	def __init__(self, ROBOT_MODEL='', ROBOT_ID='dsr01'):
		self.ROBOT_MODEL = ROBOT_MODEL
		self.ROBOT_ID = ROBOT_ID
		self.cnt = 0
		self.robot_config['name'] = ROBOT_MODEL
		self.init_Ros(ROBOT_MODEL, ROBOT_ID)
		self.robot_state_num = 0
		self.robot_state_str = ''
		# self.robot = CDsrRobot(ROBOT_ID,ROBOT_MODEL)

	def init_Ros(self, ROBOT_MODEL='', ROBOT_ID='dsr01'):
		rospy.init_node(ROBOT_ID + '_' + ROBOT_MODEL + '_slave')
		...
		self.move_spline_joint = rospy.ServiceProxy('/' + ROBOT_ID + ROBOT_MODEL + "/motion/move_spline_joint", MoveSplineJoint)
		self.move_spline_pos = rospy.ServiceProxy('/' + ROBOT_ID + ROBOT_MODEL + "/motion/move_spline_task", MoveSplineTask)
		self.move_blend = rospy.ServiceProxy('/' + ROBOT_ID + ROBOT_MODEL + "/motion/move_blending", MoveBlending)

		self.digital_out = rospy.ServiceProxy('/' + ROBOT_ID + ROBOT_MODEL + '/io/set_digital_output', SetCtrlBoxDigitalOutput)
		self.analog_input = rospy.ServiceProxy('/' + ROBOT_ID + ROBOT_MODEL + "/io/get_analog_input", GetCtrlBoxAnalogInput)
		self.set_mode_analog_input = rospy.ServiceProxy('/' + ROBOT_ID + ROBOT_MODEL + "/io/set_analog_input_type", SetCtrlBoxAnalogInputType)
	...
	def thread_subscriber(self):
		rospy.Subscriber('/' + self.ROBOT_ID + self.ROBOT_MODEL + '/state', RobotState, self.get_robot_state)
		rospy.spin()
...
if __name__ == '__main__':
	...
	m_robot_slave = Robot_Slave(ROBOT_MODEL='a0509')
